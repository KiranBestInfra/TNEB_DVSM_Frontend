...
const TNEBDashboard = () => {
  ...
  const totalCommunicated = regions.reduce(
    (sum, region) => sum + region.communicated,
    0
  );
  const totalNonCommunicated = regions.reduce(
    (sum, region) => sum + region.nonCommunication,
    0
  );
  const communicationPercentage = Math.round(
    (totalCommunicated / (totalCommunicated + totalNonCommunicated)) * 100
  );
  ...
  const adjustedCurrentIndex =
    currentIndex + Math.min(visibleItems, regions.length);
    ...
    return (
      <>
          ...
          <div className={styles.stat_card_left}>
            ...
            <div className={styles.commincation_status}>
              {/* <p className={styles.stat_number}>
                {region.communicated + region.nonCommunication}
              </p> */}
              <div></div>
            </div>
          </div>
        ...
        <div
          ...
          style={{ cursor: "pointer" }}
        >
          <div className={styles.edc_section}>
            <span>
              EDC: <strong>{region.edc}</strong>
            </span>
          </div>
          <div className={styles.communication_status_container}>
            <div className={styles.communicating_status}>
              ...
              <div className={styles.meter_status}>
                <span
                  className={`${styles.status_indicator} ${styles.communicating}`}
                />
              </div>
              <span>
                <span className={styles.status_indicator}></span>{" "}
                <strong>{region.communicated}</strong>
              </span>
            </div>
            <div className={styles.communicating_status}>
              <div className={styles.meter_status}>
                <span
                  className={`${styles.status_indicator} ${styles.non_communicating}`}
                />
              </div>

              <span>
                {" "}
                <strong>{region.nonCommunication}</strong>
              </span>
            </div>
          </div>
        </div>
        ...
        series: [
          {
            ...
            data: Array.from({ length: 7 }, () =>
              Math.floor(region.communicated * (0.85 + Math.random() * 0.3))
            ),
          },
          {
            name: "Non-Communicating Meters",
            data: Array.from({ length: 7 }, () =>
              Math.floor(region.nonCommunication * (0.7 + Math.random() * 0.5))
            ),
          },
        ],
      },
        ...
        series: [
          {
            ...
            data: Array.from({ length: 6 }, () =>
              Math.floor(region.communicated * (0.75 + Math.random() * 0.5))
            ),
          },
          {
            name: "Non-Communicating Meters",
            data: Array.from({ length: 6 }, () =>
              Math.floor(region.nonCommunication * (0.6 + Math.random() * 0.8))
            ),
          },
        ],
      },
  ...
  const selectedRegion =
    regions[
      currentIndex >= 0 && currentIndex < regions.length ? currentIndex : 0
    ];
        ...
        <div className={styles.summary_card}>
          ...
          <div className={styles.meter_communication}>
            <div className={styles.summary_item}>
              ...
              <div className={styles.summary_progress}>
                <div
                  className={`${styles.summary_value} ${styles.negative_value}`}
                >
                  <div>96</div>
                  <div className={styles.meter_percentage}>
                    {nonCommunicationPercentage}%
                  </div>
                </div>
                <div className={styles.progress_bar}>
                  <div
          ...
          <div
            ...
            style={{
              transform: `translateX(calc(-${
                adjustedCurrentIndex * (100 / displayItems.length)
              }%))`,
            ...
            {displayItems.map((region, index) => (
              <div
                ...
                className={`${styles.total_units_container} ${
                  index === adjustedCurrentIndex ? styles.active_card : ""
                }`}
                style={{
                  width: `calc(${100 / displayItems.length}% - ${
                    (displayItems.length - 1) / displayItems.length
                  }rem)`,
                }}
              >
        ...
        <div className={styles.section_header}>
          ...
          <div className={styles.region_metrics_container}>
            ...
            <div className={styles.substation_name}>
              <div className={styles.substation_icon}>
                {" "}
                {selectedRegion.substation}
              </div>
              Substations
            </div>
          </div>
